...
task("TASK_DEPLOY_XERC1155WithURIs").setAction(async function (
  ...
  const deployContract = "XERC1155WithURIs";

  console.log("Contract Deployment Started ");
  const Erc1155 = await hre.ethers.getContractFactory("XERC1155WithURIs");
  const erc1155 = await Erc1155.deploy("Contract URI for chainId " + chainId, gatewayContract, feePayerAddress);
